...
def lca(node1, node2):
    ...
    if not isinstance(node1, Node) or not isinstance(node2, Node):
        error_message = 'Types of parameters are incorrect. They should be ob type Node'
        print(error_message)
        raise TypeError(error_message)

    dist_of_node1_from_root = get_distance_from_root(node1)
    dist_of_node2_from_root = get_distance_from_root(node2)
    ...
    lca_node = get_lca_of_two_same_leveled_node(node1, node2)
    return lca_node


def get_lca_of_two_same_leveled_node(node1, node2):
